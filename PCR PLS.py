# PCR_PLS_benchmark.py
# Same validation design as your baseline:
# - 20% holdout drawn ONLY from the UNSEEN subset
# - Bin-stratified K-fold CV on the remaining training pool
# - Step 1: PC1..PC3 correlations (train only; no leakage)# - Step 2–3: Compare PCR (Linear/Ridge) and PLS with identical CV; tune n_components
# - Step 4: Evaluate the winner on the UNSEEN-only 20% holdout
...
# CONFIG
# ====================
INPUT_CSV = "raw_matrix_all.csv"   # produced by your baseline-prep script
TARGET_COL = "concentration_ppb"
DATASET_COL = "dataset"
RANDOM_STATE = 42

# Baseline Lasso holdout RMSE (for delta line)
...
def main() -> None:
    ...
    # --- Shared preprocessor for models ---
    ...
    # Phase 1: quick compare with defaults (same splits)
    ...
    print("\n=== 20% Holdout Evaluation (UNSEEN-only) — {0} ===".format(best_model_name))
